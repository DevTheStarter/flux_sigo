...
5. **The SIGO password never reaches the server.** It lives in the browser's localStorage only. See §8.3.
...
Quick filters from the counters layer on top of the active view and are cleared with a link.
...
The SIGO password is **never** included in copied text. After copying, the button shows "Copiado" for 1.8 s. If any variable is unset, the toast says how many are missing.
...
There is deliberately **no** `acoes` table and **no** `registos` table. `acao_ref` fields hold the source record id as text.
...
| `CRM_TOKEN` | Base de dados | yes | server, encrypted |
...
### 8.3 SIGO password, browser only

`CRM_TOKEN` must be on the server because the server reads the board every 15 minutes. `SIGO_PALAVRA` is only needed when an instruction is copied, which happens in the client. So it never leaves the machine.

- Stored in `localStorage` under one key, JSON-encoded, try/catch-guarded.
- Loaded into memory at login. Never included in any request. Add a test that fails if it appears in a request body or header.
- Credenciais shows a pill "só neste navegador" on that row, and a button "Apagar deste navegador".
- Lost when the browser is cleared or the user changes computer. The UI says so.

**Consequence to state in the contract:** a breach on our side cannot expose any entity's SIGO access.

### 8.4 Encryption at rest

AES-256-GCM, key from `CHAVE_CIFRA` (64 hex chars), never in the repository. Encrypted values are never returned to the client, not even masked; the client sees connection state only.
...
`/api/cron/prazos`, hourly, `CRON_SECRET` required. For each tenant, derives the board and emits `prazo` and `bloqueio` events for cards that changed state since the last run. Keep a small `ultimo_estado` cache per `acao_ref` to detect transitions; it is state about our notifications, not about the entity's data.
...
Black and white. Off-white and near-black tokens on `:root`, dark theme under `prefers-color-scheme` and `[data-theme]`, theme choice persisted in localStorage. The only colour is a muted red (`#b4443c` light / `#e08a84` dark, with pale backgrounds), used only for `late`, `error`, `blocked` and their counters. Inter, two weights. Minimal borders (0.5px), no shadows except dropdowns.
...
**Secrets**
- [ ] `SIGO_PALAVRA` never appears in any request — tested
- [ ] `CRM_TOKEN` and `fonte_credencial` AES-256-GCM, never returned to client
...
- **Legal documents** (`docs/02-rgpd.md`, `06-contrato-subcontratacao.md`, `07-politica-privacidade.md`, `08-registo-tratamentos.md`): not development documents, but the Privacidade tab and the contract download must stay consistent with §9 and §8.3.
